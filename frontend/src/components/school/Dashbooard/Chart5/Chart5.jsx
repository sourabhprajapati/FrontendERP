...
const Chart5 = () => {
  ...
  return (
    <div className="upd-chart-container">
      <div className="upd-chart-box">
        <Line data={feesData} options={feesOptions} />
      </div>
      <div className="upd-events-section">
        <div className="upd-event-box">
          <div className="upd-tab-header">
            <div className="upd-tab-buttons">
              <button
                className={`upd-tab-button ${activeTab === 'student-birthdays' ? 'upd-active' : ''}`}
                onClick={() => setActiveTab('student-birthdays')}
              >
                Student Birthdays
              </button>
              <button
                className={`upd-tab-button ${activeTab === 'staff-birthdays' ? 'upd-active' : ''}`}
                onClick={() => setActiveTab('staff-birthdays')}
              >
                Staff Birthdays
              </button>
              <button
                className={`upd-tab-button ${activeTab === 'staff-anniversaries' ? 'upd-active' : ''}`}
          ...
          <div className="upd-event-header">
            <span className="upd-event-icon">{currentConfig.icon}</span>
            <h3>{currentConfig.title}</h3>
            <span className="upd-today-badge">{currentConfig.badge}</span>
          </div>
          <ul className="upd-event-list">
            {currentData.map((item, index) => (
              <li
                key={index}
                className={`upd-event-item ${animate ? 'upd-slide-up-item' : ''}`}
                style={{ animationDelay: `${index * 0.15}s` }} // staggered delay
              >
                <img src={item.image} alt={item.name} className="upd-event-image" />
                <div className="upd-event-details">
                  <span className="upd-event-name">{item.name}</span>
                  <span className="upd-event-date">{item.date}</span>
                </div>
              </li>
            ))}
          </ul>
          <button className="upd-wish-button">{currentConfig.buttonText}</button>
        </div>
      </div>
    </div>
  );
